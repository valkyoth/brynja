...
Status: v0.20.0 published; milestones through v0.24.38 signed. The v0.24.39 implementation adds default-off TupleHash/TupleHashXOF execution with separate public and hardened owners, exact item writers and exclusively borrowed XOF readers. See [TupleHash execution](tuplehash-accelerated-execution.md). Focused development checks passed; exceptional owner pentest, fresh native collection and final release checks are pending. No publication, independent verification or FIPS validation is claimed.
