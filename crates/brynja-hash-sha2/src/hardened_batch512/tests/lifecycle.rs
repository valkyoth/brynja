...
fn focused_unwind_after_secret_processing_clears_and_revokes() -> Result<(), Error> {
    let b = bits(&[0x63; 256], 8)?;
    let inputs = [Some(Input::new(Algorithm::Sha512, b)), None, None, None];
    for secret in [false, true] {
        ...
        assert_eq!(storage.first(), Some(&[if secret { 0 } else { 0xa5 }; 64]));
        assert!(storage.iter().skip(1).all(|slot| *slot == [0xa5; 64]));
        ...
        for bad_slot in 0..CAPACITY {
            for width in 0..=65 {
                ...
                let inputs = core::array::from_fn(|index| {
                    (active || index != bad_slot).then(|| Input::new(Algorithm::Sha512, b))
                });
                for secret in [false, true] {
                    let executor = Executor::portable();
                    ...
                    let dest = storage.each_mut().map(|out| {
                        let size = if index == bad_slot { width } else { 64 };
                    ...
                    let mut control = Control::new(4, &mut cancel);
                    if secret {
                        assert!(matches!(
...
fn work_failures(executor: &Executor<'_>) -> Result<(), Error> {
    ...
    for budget in 0..12 {
        for secret in [false, true] {
            ...
            assert_eq!(storage, [[if secret { 0 } else { 0xa5 }; 64]; CAPACITY]);
...
fn declassification_is_consuming_atomic_and_clears_originals() -> Result<(), Error> {
    ...
    for bad in [false, true] {
        let mut secret = [[0xa5; 64]; CAPACITY];
        let mut public = [[0x69; 64]; CAPACITY];
        let mut workspace = Workspace::new();
        let mut cancel = || false;
        let mut control = Control::new(4, &mut cancel);
        ...
        assert_eq!(secret, [[0; 64]; CAPACITY]);
        if bad {
            assert_eq!(result, Err(Error::OutputShape));
            assert_eq!(public, [[0x69; 64]; CAPACITY]);
        } else {
            result?;
