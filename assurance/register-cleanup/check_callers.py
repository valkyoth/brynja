...
PROFILES = ('movable', 'scoped', 'higher', 'accelerated', 'threaded')
ACCELERATED_PACKAGES = (*HIGHER_PACKAGES, 'brynja-hash-sha3', 'brynja-crypto-cpu')
THREADED_PACKAGES = ('brynja-hash-parallel-std', 'brynja-crypto-cpu-std')
THREADED_ALGORITHMS = {route + form + strength for route in ('portable', 'static', 'batch')
                       for form in ('', 'xof') for strength in ('128', '256')}
...
def sources():
    paths = {ROOT / 'Cargo.toml', ROOT / 'Cargo.lock', Path(__file__).resolve(),
             ...
             Path(__file__).with_name('check_threaded_callers.py')}
    ...
    for directory in [FIXTURE, *(ROOT / 'crates' / name for name in (*PACKAGES, *HIGHER_PACKAGES, *THREADED_PACKAGES, 'brynja-crypto-cpu'))]:
...
def observations(log, api_profile='movable', kernel=None):
    ...
    algorithms = THREADED_ALGORITHMS if api_profile == 'threaded' else HIGHER_ALGORITHMS if api_profile in ('higher', 'accelerated') else ALGORITHMS
    ...
    expected_cases = 18 if api_profile == 'threaded' else 28
    if any(int(cases) != expected_cases or not 0 <= int(matches) <= expected_cases
           for _, cases, matches in rows):
        raise ValueError('incorrect diagnostic case counts')
    count = 6 if api_profile in ('accelerated', 'threaded') else 5
    ...
    if api_profile == 'threaded':
        routes = re.findall(r'^CALLER_THREADS: kernel=(\w+); workers=2; block=8; profiles=portable,static,batch; coordinator_only=true$', log, re.MULTILINE)
        if kernel not in ('X86Keccak', 'ArmKeccak') or routes != [kernel]:
            raise ValueError('missing/duplicate/wrong threaded coordinator scope or routes')
...
def emitted_command(compiler, common, package, api_profile):
    if api_profile not in PROFILES or api_profile == 'threaded':
...
def emitted_threaded_tree(compiler, common):
    # RUSTFLAGS supplies --emit for every dependency in this exact unified graph.
    return ['cargo', '+' + compiler, 'build', *common, '--features', 'threaded', '--lib']
...
def main():
    ...
    mode.add_argument('--threaded', action='store_true', help='observe ParallelHash coordinators, not worker registers')
    args = parser.parse_args()
    api_profile = 'threaded' if args.threaded else 'accelerated' if args.accelerated else 'higher' if args.higher else 'scoped' if args.scoped else 'movable'
    ...
    for compiler in ('1.90.0', '1.98.1'):
        for target in targets:
            for profile in ('debug', 'release'):
                ...
                if args.emit:
                    packages = (*PACKAGES, *(HIGHER_PACKAGES if args.higher or args.accelerated or args.threaded else ()),
                                *(THREADED_PACKAGES if args.threaded else ()),
                                *(('brynja-crypto-cpu',) if args.accelerated or args.threaded else ()), 'brynja-caller-residue-audit')
                    if args.threaded:
                        emit_env = dict(env)
                        emit_env['RUSTFLAGS'] += ' --emit=mir,llvm-ir,asm,link'
                        run(emitted_threaded_tree(compiler, common), emit_env)
    ...
    report = {'schema': 1, 'qualifies_register_cleanup': False,
              ...
              'scope': ('threaded coordinator (not exited workers)' if args.threaded else 'static accelerated' if args.accelerated else 'portable') + ' public wrapper normal return; selected volatile registers only',
