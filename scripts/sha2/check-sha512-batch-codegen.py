...
def inspect(assembly, target):
    ...
    if architecture == 'x86':
        required = (r'\bvpaddq\b', r'\bvpxor\b', r'\bvpsrlq\b', r'\bymm\d+\b')
        forbidden = r'\bsha512\w+\b'
    else:
        required = (r'\badd(?:\s+v\d+\.2d|\.2d\s+v\d+)',
                    r'\beor(?:3)?(?:\s+v\d+\.|\.16b\s+v\d+)',
                    r'\bushr(?:\s+v\d+\.2d|\.2d\s+v\d+)')
        forbidden = r'\bsha512\w+\b'
...
def check(target, toolchain='1.98.1'):
    with tempfile.TemporaryDirectory(prefix='brynja-sha512-batch-codegen-') as directory:
        ...
        acceptance.run(['cargo', '+' + toolchain, 'rustc', '--locked', '--offline', '--release',
                        '-p', 'brynja-crypto-cpu', '--features', 'sha512-batch', '--target', target,
                        '--', '--emit=asm'], env=env)
        ...
        for before, after in (('vpaddq', 'scalar_add'), ('vpxor', 'scalar_xor')) if target.startswith('x86') else (('eor', 'scalar_xor'), ('ushr', 'scalar_shift')):
    ...
    print(f'SHA-512-family independent SIMD codegen: PASS; {toolchain}; {target}')
