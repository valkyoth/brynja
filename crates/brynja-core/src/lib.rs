...
pub use security_event::{
    SecurityEvent, SecurityEventDropCount, SecurityEventKind, SecurityEventPush,
    SecurityEventQueue, SecurityEventQueueSnapshot, SecurityEventRecord, SecurityEventTimestamp,
    SecurityEventTimestampError,
};
pub use security_outcome::{
    ...
    ProtocolSelectionDecision, ProviderDecision, PskDecision, ResumptionDecision, SecurityAccepted,
    SecurityApproved, SecurityAuthority, SecurityAuthorityError, SecurityAuthoritySnapshot,
    SecurityAuthorityState, SecurityCanceled, SecurityDecision, SecurityDecisionKind,
    SecurityDisposition, SecurityFailed, SecurityFailure, SecurityFailureKind, SecurityNonApproved,
    SecurityOutcome, SecurityPending, SecurityReceipt, SecurityRejected, SecurityRejection,
...
/// Whether the v0.18.1 bounded observational security-event schema is implemented.
...
mod tests {
    ...
    fn package_claims_only_completed_foundation_domains() {
        ...
        assert!(::core::hint::black_box(
            super::SECURITY_EVENT_SCHEMA_IMPLEMENTED
        ));
    }
}
