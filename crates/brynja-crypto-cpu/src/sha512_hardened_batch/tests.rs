...
fn pack_rejects_invalid_width_and_initializes_inactive_lanes() -> Result<(), Error> {
    let mut workspace = Workspace::new();
    let states = [[0x123456789abcdef0; 8]; 4];
    let blocks = [[0x91; 128]; 4];
    for width in [0, 1, 3, 5, 7, 8, 9, usize::MAX] {
        poison(&mut workspace);
        assert_eq!(
        ...
        poison(&mut workspace);
        assert_eq!(
            workspace.pack_bytes(&[[0x12; 64]; 4], &blocks, width),
            Err(Error::Invariant)
        );
        cleared(&workspace);
    }
    poison(&mut workspace);
    workspace.pack(&states, &blocks, 2)?;
...
fn rejection_and_unwind_clear_before_any_instruction() {
    let mut workspace = Workspace::new();
    let mut states = [[0xdeadbeef12345678; 8]; 4];
    let blocks = [[0x7c; 128]; 4];
    for panic in [false, true] {
        ...
        assert_eq!(states, [[0xdeadbeef12345678; 8]; 4]);
...
fn reference(state: &mut [u64; 8], block: &[u8; 128]) -> Result<(), Error> {
    ...
    for (word, constant) in w.into_iter().zip(crate::sha512_schedule::ROUND_CONSTANTS) {
        let t1 = h
...
fn native_lane_distinct_differential_and_actual_dispatch() -> Result<(), Error> {
    let mut known = crate::sha512::initial_state();
    reference(&mut known, &crate::sha512::abc_block())?;
    assert_eq!(known, crate::sha512::abc_digest_state());
    ...
    for kernel in [Kernel::Avx2, Kernel::Neon] {
        ...
        for _ in 0..512 {
            ...
            for (state, words) in byte_states.iter().zip(expected) {
                for (bytes, word) in state.as_chunks::<8>().0.iter().zip(words) {
    ...
    if std::env::var_os("BRYNJA_REQUIRE_SHA512_HARDENED_BATCH").is_some() {
    ...
    for kernel in [Kernel::Avx2, Kernel::Neon] {
        for boundary in 0..=1 {
            ...
            for panic in [false, true] {
                ...
                let mut workspace = Workspace::new();
                poison(&mut workspace);
                let mut states = [[0xa5; 64]; 4];
                let outcome = catch_unwind(AssertUnwindSafe(|| {
                    session.compress_bytes(&mut states, &[[0x96; 128]; 4], &mut workspace)
                }));
                if panic {
                    assert!(outcome.is_err());
                } else {
