#!/usr/bin/env python3
"""Source-owned TupleHash execution cleanup under the pinned compiler endpoints."""
import argparse
...
def require(value, label):
    if not value:
        raise ValueError('TupleHash execution cleanup: ' + label)
...
def check(row):
    ...
    require(Counter(map(int, widths)) == Counter((1, 1, 16, 16, 16, 16, 1, 168)), 'exact emitted region widths')
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='brynja-tuplehash-cleanup-') as directory:
        ...
        subprocess.run(['cargo', '+' + args.toolchain, 'rustc', '--locked', '--offline',
            '-p', 'brynja-hash-tuple', '--features', 'hardened-execution,runtime-execution',
            '--release', '--lib', '--target', args.target, '--', '--emit=mir,llvm-ir,asm'],
            cwd=ROOT, env=env, check=True, timeout=180)
        row = {}
        for extension in ('mir', 'll', 's'):
            paths = list(Path(directory).rglob('brynja_hash_tuple-*.' + extension))
        ...
        for before in ('Metadata::wipe(', '= clear_owned_region('):
            ...
            try:
                ...
                continue
            raise ValueError('TupleHash emitted cleanup mutant escaped')
    print(f'TupleHash execution MIR/LLVM/assembly cleanup: PASS; {args.toolchain} {args.target}')
