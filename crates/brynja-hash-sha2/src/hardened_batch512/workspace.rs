...
///
/// ```compile_fail
/// use brynja_hash_sha2::hardened_batch512::Workspace;
/// fn require<T: Send>() {}
/// require::<Workspace>();
/// ```
///
...
pub struct Workspace {
    pub(super) states: [[u8; 64]; CAPACITY],
    pub(super) packed: [[u8; 64]; CAPACITY],
    pub(super) blocks: [[u8; 128]; CAPACITY],
    pub(super) output: [[u8; 64]; CAPACITY],
    pub(super) offsets: [[u8; 8]; CAPACITY],
    pub(super) indices: [u8; CAPACITY],
    pub(super) active: [u8; 1],
    pub(super) scalar: HardenedSha2Owner,
    pub(super) cpu: brynja_crypto_cpu::sha512_hardened_batch::Workspace,
}
...
impl Workspace {
    ...
    pub fn new() -> Self {
        Self {
            states: [[0; 64]; CAPACITY],
            packed: [[0; 64]; CAPACITY],
            blocks: [[0; 128]; CAPACITY],
            output: [[0; 64]; CAPACITY],
            offsets: [[0; 8]; CAPACITY],
            indices: [0; CAPACITY],
            active: [0],
            scalar: HardenedSha2Owner::new64([0; 8]),
            cpu: brynja_crypto_cpu::sha512_hardened_batch::Workspace::new(),
        }
    }
    #[inline(never)]
