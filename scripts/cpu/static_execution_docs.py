...
def validate(root):
    ...
    words += ['fifty', 'fifty-one', 'fifty-two', 'fifty-three']
...
def regressions(root):
    ...
    cases += [
        (DOCS[0], 'Status: fifty-three exact', 'Status: nine exact'),
        (DOCS[0], 'Rust in only fifty-three exact', 'Rust in only nine exact'),
