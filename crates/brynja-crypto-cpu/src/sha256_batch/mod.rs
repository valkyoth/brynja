//! Default-off independent-message SHA-224/256 SIMD compression.
//!
//! Ordinary/public data only: no erasure, hashing, padding or secret ownership.
//! AVX2 processes eight lanes; NEON processes the first four (leaving the rest
...
pub enum Kernel {
    /// Eight independent 32-bit word lanes; AVX2 and OS-enabled YMM required.
    Avx2,
    /// Four independent 32-bit word lanes; AArch64 NEON required.
    Neon,
}
...
///
/// ```compile_fail
/// use brynja_crypto_cpu::sha256_batch::Authority;
...
impl Authority {
    ...
    fn create(kernel: Kernel, revalidate: fn(Kernel) -> bool) -> Result<Self, Error> {
        ...
        let mut states = core::hint::black_box([crate::sha256::initial_state(); 8]);
        let blocks = core::hint::black_box([crate::sha256::abc_block(); 8]);
        owner
            .session()?
        ...
        if !states
            .iter()
            .take(kernel.width())
            .all(|state| *state == crate::sha256::abc_digest_state())
        {
            owner.quarantine();
...
impl Session<'_> {
    ...
    /// NEON leaves slots four through seven unchanged. No secret-state clearing.
    pub fn compress(
        &self,
        states: PublicData<&mut [[u32; 8]; 8]>,
        blocks: PublicData<&[[u8; 64]; 8]>,
    ) -> Result<(), Error> {
        let mut guard = Operation {
            owner: self.owner,
            complete: false,
