...
pub(super) fn inject(fault: Fault, point: Boundary, owner: &Authority, cancel: &Cancellation) {
    ...
    match fault {
        ...
        Fault::Panic(at) if at == point => {
            std::panic::resume_unwind(Box::new("strict tuple fault"))
        }
        ...
        Fault::PanicSecondOutput if second => {
            std::panic::resume_unwind(Box::new("strict tuple XOF fault"))
        }
        _ => (),
    }
}
pub(super) fn declared_bits(fault: Fault, bits: u128) -> u128 {
    if matches!(fault, Fault::WrongLength) {
...
pub(super) fn observe(
    ...
    output: &[u8],
    owner: &Authority,
    session: &brynja_hash_tuple::execution::KeccakSession<'_>,
) {
...
#[test]
fn compiled_admission_rejects_wrong_operation_target_and_model() {
    let _ = [
        Fault::Cancel(Boundary::Item),
        Fault::Revoke(Boundary::Item),
        Fault::Panic(Boundary::Item),
        Fault::WrongLength,
    ...
    for kernel in [Kernel::X86Keccak, Kernel::ArmKeccak] {
        if let Err(error) = super::super::require_target() {
            assert!(
                matches!(CompiledSession::new(Algorithm::TupleHash256(256), kernel, limits()), Err(e) if e == error)
            );
        } else if let Err(error) = kernel.check_compiled_target() {
            assert!(
                matches!(CompiledSession::new(Algorithm::TupleHash256(256), kernel, limits()), Err(Error::Backend(e)) if e == error)
            );
        }
    }
}
