...
fn reference(out: &mut [u8], input: &[u8], words: usize, pack: bool, width: usize, swap: u32) {
    for lane in 0..width {
        for word in 0..words {
            let packed = word * 32 + lane * 4;
            let ordinary = (lane * words + word) * 4;
            let (src, dst) = if pack {
                (ordinary, packed)
            } else {
                (packed, ordinary)
            ...
            for byte in 0..4 {
                out[dst + byte] = input[src + if swap == 0 { byte } else { 3 - byte }];
            }
        }
    }
}
...
fn transposition_bounds_and_register_cleanup() {
    ...
    assert_eq!(cases, 2592);
    std::println!("SHA256_TRANSFER: 2592 layouts; register cleanup and canaries: PASS");
}
...
fn guarded_transfers() -> std::io::Result<()> {
    for (words, pack, function) in CASES {
        for source_end in [false, true] {
            for destination_end in [false, true] {
                ...
                reference(&mut expected[..length], src, words, pack, 8, 1);
                source.readonly()?;
                let src = if words == 8 {
                ...
                // SAFETY: Exactly sized, disjoint live page views; width=8.
                assert_eq!(unsafe { capture(function, dst, src, 8, 1,) }, 0);
    ...
    std::println!("SHA256_TRANSFER_BOUNDS: 12 guarded placements; readonly input: PASS");
    Ok(())
}
