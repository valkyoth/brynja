use super::*;
use brynja_crypto_cpu::sha512_hardened_batch::Error as CpuError;
...
fn actual_hardened_routes_clear_outputs_and_never_recover_revoked_authority() -> Result<(), Error> {
    ...
    let bits = brynja_hash_sha2::BitString::new(&message, 8).map_err(|_| Error::Unavailable)?;
    let inputs = core::array::from_fn(|_| Some(Input::new(Algorithm::Sha512, bits)));
    let expected = brynja_hash_sha2::sha512(&message).map_err(|_| Error::Unavailable)?;
    for mode in [Mode::Portable, Mode::Prefer, Mode::Require] {
        ...
        let mut outputs = [[0xff; 64]; CAPACITY];
        ...
        assert_eq!(outputs, [[0; 64]; CAPACITY]);
        ...
        assert_eq!(outputs, [[0xa5; 64]; CAPACITY]);
    }
    Ok(())
}
