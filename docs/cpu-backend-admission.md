...
- OS, kernel, virtualization, compiler and commit, target, flags, clock,
  frequency policy, and isolation;
...
- one size and SHA-256 for every raw harness artifact plus an explicit residual
  statement that statistical side-channel testing is not proof.
...
incomplete or overbroad feature sets, architecture disagreement, noisy data,
and benchmark-order bias are rejected.
...
exact native artifacts and passes the v0.13.2 unsafe-boundary amendment,
