...
Status: v0.20.0 published; milestones through v0.24.43 signed. The v0.24.44 candidate adds distinct default-off [hardened MD5 batch SIMD](legacy-md5-hardened-execution.md), clearing packed storage, affine batches, typed secret outputs and static/hosted selection. The supplied retest and all four native lanes for both ordinary and hardened execution passed; final release verification and GitHub checks remain required before tagging. The cleanup deferral is addressed within the documented source-owned contract, not by claiming register/platform erasure or making ordinary SIMD secret-safe. The signed v0.24.43 baseline is 4640d7c7. No crates.io publication, independent verification or FIPS validation is claimed.
...
jobs with positive worker and maximum-leaf limits, one fail-closed operation at
a time per executor, worker-sized reusable storage, pre-allocation
cancellation, fallible scoped thread creation, deterministic joins, panic
containment, and no edge into defaults, facades, bare-metal, or FIPS
...
with zero open findings. The overall SP 800-185 family is now **Fully implemented**
after v0.24.17 backend, native-parallel, timing, performance and final evidence disposition.
...
unadmitted; the family table reflects the completed technical acceptance in the
[native-evidence checklist](sp800185-final-acceptance.md).
The owner-supplied review of v0.24.16 through c58711b found no Critical, High,
Medium or Low findings. This is a security-assessment PASS, not independent
cryptographic verification or permission to tag. AMD, AWS Intel, AWS AArch64
and Apple M2 observations pass
and are [archived with their source binding](../assurance/sp800185-observations/v0.24.17/README.md);
all required local release-check stages passed. Green GitHub/CodeQL and explicit
tag authorization remain outstanding; no crates.io publication is selected.
It also
...
observations plus performance, migration, side-channel, independent-review,
secret-erasure, and FIPS-validation evidence remain pending or absent; both
candidates stay unadmitted and zero crates are selected for publication. The
repository owner reported the exceptional assessment of exact candidate
...
- the generated closure maps all 130 locked sources, all 522 ordered pre-1.0
