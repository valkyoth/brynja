...
def main():
    ...
    cases += [(policy.SOURCE + 'ownership.rs', case, '') for case in policy.DOC_CASES]
    ...
    with tempfile.TemporaryDirectory(prefix='brynja-tuplehash-policy-') as directory:
        root = Path(directory)
    ...
    print(f'TupleHash execution policy rejects {len(cases)} semantic regressions')
