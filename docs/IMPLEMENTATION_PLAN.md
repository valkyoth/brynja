...
├── brynja-mac-kmac (complete KMAC/KMACXOF ownership over hardened cSHAKE)
├── brynja-hash-tuple (complete TupleHash/TupleHashXOF ownership over hardened cSHAKE)
├── brynja-hash-parallel (complete no_std ParallelHash/ParallelHashXOF ownership)
├── optional brynja-hash-parallel-std (bounded native leaf-job executor)
...
the v0.24.22 freshness re-review advances the current exact pin to 2.1.0
