...
/// ```compile_fail
/// struct Forged;
/// impl brynja_legacy_sha1::HardenedSha1State for Forged {}
/// ```
pub trait HardenedSha1State: sealed::Sealed {}

/// Secret-bearing legacy SHA-1 with mandatory clearing of its private regions.
///
/// Hardening memory does not repair SHA-1 collisions or make raw SHA-1 a MAC.
...
/// ```compile_fail
/// let state = brynja_legacy_sha1::HardenedSha1::new();
/// let _ = state.clone();
/// ```
/// ```compile_fail
/// let state = brynja_legacy_sha1::HardenedSha1::new();
/// println!("{:?}", state);
/// ```
pub struct HardenedSha1 {
    owner: Sha1Owner,
}
impl sealed::Sealed for HardenedSha1 {}
impl HardenedSha1State for HardenedSha1 {}

impl HardenedSha1 {
    /// Creates an empty secret-bearing SHA-1 state.
    ...
    /// Finalizes into public storage of exactly 20 bytes; errors preserve it.
    pub fn finalize_public(
        self,
        destination: &mut [u8],
        authority: PublicDeclassification,
    ) -> Result<(), Sha1Error> {
        let empty = BitString::new(&[], 0).map_err(|_| Sha1Error::MessageTooLong)?;
    ...
    pub fn finalize_secret(
        ...
        match BitString::new(&[], 0) {
            ...
            Err(_) => Err(output::failed(destination, Sha1Error::MessageTooLong)),
        }
    }
