...
def main():
    ...
    mutations = [
        ...
        (policy.CRATE + 'src/engine.rs',
         ...
         'let count = owner.block.len().saturating_sub(offset).min(input.len());\n        assert!(offset < owner.block.len(), "SHA-1 update offset invariant");'),
        (policy.CRATE + 'src/engine.rs', 'tail.split_borrowed()', 'tail.split()'),
        (policy.CRATE + 'src/engine.rs', 'copy_secret_region(destination, source)', 'copy_secret_region(destination, &[])'),
        (policy.CRATE + 'src/engine.rs', '0x80 >> valid', '0x40 >> valid'),
        (policy.CRATE + 'src/engine.rs', 'core::slice::from_ref(byte)', 'core::slice::from_ref(&0)'),
        (policy.CRATE + 'src/engine.rs', 'copy_secret_region(&mut owner.output_staging, &owner.chaining_state)', 'copy_secret_region(&mut owner.output_staging, &[])'),
