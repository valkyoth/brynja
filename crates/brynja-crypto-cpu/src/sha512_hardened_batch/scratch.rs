...
///
/// ```compile_fail
/// use brynja_crypto_cpu::sha512_hardened_batch::Workspace;
/// fn require<T: Send>() {}
/// require::<Workspace>();
/// ```
/// ```compile_fail
    ...
    pub(super) fn pack_bytes(
        ...
        for (word, packed) in self.initial.iter_mut().enumerate() {
                ...
                .take(width)
            {
                ...
                slot.copy_from_slice(&u64::from_be_bytes(*bytes).to_ne_bytes());
            }
        }
        ...
        for (lane, state) in states.iter_mut().enumerate().take(width) {
            for (out, packed) in state.as_chunks_mut::<8>().0.iter_mut().zip(&self.work) {
                for (index, bytes) in packed.as_chunks::<8>().0.iter().enumerate() {
                    if index == lane {
                        out.copy_from_slice(&u64::from_ne_bytes(*bytes).to_be_bytes());
                    }
                }
            }
        }
    ...
    pub(super) fn commit(&self, states: &mut [[u64; 8]; 4], width: usize) {
        for (lane, state) in states.iter_mut().enumerate().take(width) {
            for (out, packed) in state.iter_mut().zip(&self.work) {
                // Iteration over the same fixed four slots makes this total.
                for (index, bytes) in packed.as_chunks::<8>().0.iter().enumerate() {
                    if index == lane {
