    ...
    def test_compiled_algorithm_mutants_are_rejected(self):
        ...
        with tempfile.TemporaryDirectory(prefix="brynja-general-mutants-") as directory:
            ...
            mutants = (
                ...
                ("hardened.rs", "brynja_core::apply_secret_byte_mask(last, self.parameter.last_byte_mask(), 0);",
                 "brynja_core::apply_secret_byte_mask(last, 0xff, 1);"),
            ...
            # Check the whole population before compiling any mutant, so stale
            # anchors fail promptly and identify the affected transformation.
            for name, before, _ in mutants:
                original = (root / "src/general" / name).read_text()
                self.assertEqual(original.count(before), 1, (name, before))
            ...
            print(f"General SHA-512/t algorithm mutants: {len(mutants)} compiled and rejected by runtime tests")
