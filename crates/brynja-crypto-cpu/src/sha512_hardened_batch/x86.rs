//! Four clearing SHA-512 lanes; no ordinary packed buffers are borrowed.
#![allow(unsafe_code)]
...
pub(super) unsafe fn compress_secret(s: &mut Workspace) -> Result<(), Error> {
    ...
    unsafe {
        ...
        for t in 16_usize..80 {
            ...
            store(
                sigma0,
                xor!(xor!(ror!(x, 1), ror!(x, 8)), _mm256_srli_epi64::<7>(x)),
            );
            store(
                sigma1,
                xor!(xor!(ror!(y, 19), ror!(y, 61)), _mm256_srli_epi64::<6>(y)),
            );
            let w16 = load(
                s.schedule
