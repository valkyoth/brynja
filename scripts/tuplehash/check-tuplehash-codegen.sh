...
extract_assembly_function() {
    ...
    local artifact="$2"
    awk -v pattern="$pattern" '
        /^(_ZN|_RNvMs)[^:]*:$/ && index($0, pattern) > 0 { active = 1 }
        active { print }
...
for raw_toolchain in "${toolchains[@]}"; do
    ...
    for mir in "${tuple_mir[@]}"; do
        for operation in squeeze_final_public squeeze_final_secret; do
            ...
            grep -Eq '= &mut .*as Strength128.*HardenedCshake128Reader' <<<"$finalizer"
            grep -Eq '= &mut .*as Strength256.*HardenedCshake256Reader' <<<"$finalizer"
            grep -q "${operation/squeeze_final_/squeeze_final_bits_}_erasing_source" <<<"$finalizer"
        done
        reader_drop="$(extract_function '>::drop(_1: &mut BackendReader)' "$mir")"
        grep -q 'HardenedCshake128Reader::wipe_in_place' <<<"$reader_drop"
        grep -q 'HardenedCshake256Reader::wipe_in_place' <<<"$reader_drop"
        tuple_finish="$(extract_function '>::finish(_1: &mut TupleCore' "$mir")"
