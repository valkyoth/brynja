use super::*;
use brynja_crypto_cpu::sha256_hardened_batch::Error as CpuError;
...
fn actual_hardened_routes_clear_outputs_and_never_recover_revoked_authority() -> Result<(), Error> {
    ...
    let inputs = core::array::from_fn(|_| Some(Input::new(Algorithm::Sha256, bits)));
    let expected = brynja_hash_sha2::sha256(&message).map_err(|_| Error::Unavailable)?;
