// Native normal-return boundary; other targets/Miri/Kani retain the safe model.
#[cfg(all(
    not(any(miri, kani)),
    any(
        target_arch = "x86_64",
...
mod native;
pub(crate) fn permute<const RATE: usize>(owner: &mut super::owner::HardenedFips202Owner<RATE>) {
    #[cfg(all(
        not(any(miri, kani)),
        any(
            target_arch = "x86_64",
    ...
    {
        native::permute(
            &mut owner.sponge_lanes,
            &mut owner.permutation_columns,
            &mut owner.permutation_theta,
            &mut owner.permutation_rearranged,
        );
...
mod model {
    ...
    use super::super::owner::HardenedFips202Owner;
    ...
    fn theta<const RATE: usize>(owner: &mut HardenedFips202Owner<RATE>) {
        ...
        for x in 0_usize..5 {
            ...
            let right =
                read_word(&owner.permutation_columns, x.saturating_add(1) % 5).rotate_left(1);
    ...
    fn write_lane<const RATE: usize>(
        owner: &mut HardenedFips202Owner<RATE>,
        index: usize,
        value: u64,
    ) {
