...
const _: unsafe extern "win64" fn(&mut [u8; 2752], &[u32; 64]) = compress;
...
fn prepare(seed: &mut u64, case: usize) -> ([u8; 2752], [u8; 256]) {
    let mut packed = [0xa5; 2752];
    ...
    for lane in 0..8 {
        ...
        for (word, bytes) in state[..32].as_chunks::<4>().0.iter().enumerate() {
            packed[word * 32 + lane * 4..word * 32 + lane * 4 + 4]
                .copy_from_slice(&u32::from_be_bytes(*bytes).to_le_bytes());
        }
        for (word, bytes) in block[..64].as_chunks::<4>().0.iter().enumerate() {
            packed[256 + word * 32 + lane * 4..260 + word * 32 + lane * 4]
                .copy_from_slice(&u32::from_be_bytes(*bytes).to_le_bytes());
        }
        if lane < WIDTH {
            let output = reference(state, &block);
            for (word, bytes) in output[..32].as_chunks::<4>().0.iter().enumerate() {
                expected[word * 32 + lane * 4..word * 32 + lane * 4 + 4]
                    .copy_from_slice(&u32::from_be_bytes(*bytes).to_le_bytes());
            }
        }
    }
...
fn check(scratch: &[u8; 2752], expected: &[u8; 256]) {
    assert_eq!(
        &scratch[2304..2560],
        expected,
        "lane result or inactive lane corruption"
    );
    assert_eq!(&scratch[..2304], &[0; 2304], "input/schedule residue");
    assert_eq!(&scratch[2560..], &[0; 192], "temporary residue");
}
...
fn compression_and_return_registers() {
    ...
    for offset in 1..32 {
        ...
        let mut backing = [0x39; 2783];
        let Ok(scratch) = <&mut [u8; 2752]>::try_from(&mut backing[offset..offset + 2752]) else {
        ...
        *scratch = original;
        ...
        assert!(
            backing[..offset]
                .iter()
                .chain(&backing[offset + 2752..])
                .all(|b| *b == 0x39)
        );
    }
    std::println!("BATCH256_CLEANUP: 1024 independent batches; 31 unaligned placements; PASS");
}
...
unsafe fn observe(scratch: &mut [u8; 2752], snapshot: &mut [u8; 312]) {
...
unsafe fn observe(scratch: &mut [u8; 2752], snapshot: &mut [u8; 160]) {
...
fn guarded_bounds() -> std::io::Result<()> {
    ...
    for placement in 0..4 {
        ...
        *scratch.bytes::<2752>(s_end) = initial;
        ...
        constants.readonly()?;
        let bytes = constants.read::<256>(c_end);
        assert_eq!(bytes.as_ptr().align_offset(core::mem::align_of::<u32>()), 0);
        // SAFETY: Exact aligned initialized read-only 64-word allocation.
        let words = unsafe { &*bytes.as_ptr().cast::<[u32; 64]>() };
    ...
    std::println!("BATCH256_BOUNDS: 4 guarded placements; readonly constants: PASS");
    Ok(())
}
