#!/usr/bin/env python3
"""Scalar MD5 development evidence; not a release-gate change."""
...
ROOT = HERE.parents[1]
SOURCE = ROOT / 'crates/brynja-legacy-md5/src/compress/native.rs'
FIXTURE = HERE / 'md5-scalar'
...
def inspect(text, arm):
    ...
    allowed = (r'(?:ldr|str|mov|cmp|b(?:\.hs|\.ne)?|and|bic|orr|eor|add|orn|lsl|sub|lsr|neg|ror)\b'
               if arm else r'(?:movl|xorl|cmpl|jae|jmp|jne|andl|notl|orl|leal|imull|addl|shrl|roll|incl)\b')
...
def prepare(directory):
    ...
    lib.write_text(lib.read_text().replace('../../../../crates/brynja-legacy-md5/src/compress/native.rs', 'native.rs')
                   .replace('../../../../crates/brynja-legacy-md5/src/cpu/constants.rs',
                            str(ROOT/'crates/brynja-legacy-md5/src/cpu/constants.rs')))
...
def controls(original, arm):
    ...
    for wipe in wipes:
        assert original.count(wipe) == 1
        yield wipe, poisoned.replace(wipe, ''), False
    changes = ([('"cmp w10, #64"', '"cmp w10, #63"'),
                ('"neg w9, w9"', '"mov w9, w9"'),
                ('"and w9, w9, #15"', '"and w9, w9, #7"'),
                ('"add w4, w4, w8"', '"mov w4, w8"')]
               if arm else [('"cmp r14d, 64"', '"cmp r14d, 63"'),
                            ('"rol r8d, cl"', '"ror r8d, cl"'),
                            ('"and edx, 15"', '"and edx, 7"'),
                            ('"add [{state}], r8d"', '"mov [{state}], r8d"')])
...
def main():
    with tempfile.TemporaryDirectory(prefix='brynja-md5-scalar-') as temporary:
        ...
        for compiler in ('1.90.0', '1.98.1'):
            ...
            for target in targets:
                ...
                for release in (False, True):
                    ...
                    print(f'Scalar MD5 assembly: {compiler} {target} release={release}: PASS', flush=True)
                    ...
                    run(['cargo', '+'+compiler, 'rustc', '--locked', '--offline', '-p', 'brynja-legacy-md5',
                         '--no-default-features', '--target', target, *(['--release'] if release else []),
                         '--lib', '--', '--emit=asm'], crate_env)
                    actual = list(Path(crate_env['CARGO_TARGET_DIR']).glob(f'{target}/*/deps/brynja_legacy_md5-*.s'))
                    assert len(actual) == 1
                    ...
                    for label, changed, success in controls(original, arm):
                        ...
                        if success:
                            if result.returncode or 'MD5_SCALAR: 1024 independent pairs;' not in log or 'MD5_SCALAR_BOUNDS: 16 placements;' not in log:
                        ...
                        count += 1
                    print(f'Scalar MD5 real crate + runtime: {compiler} {target} release={release}; {count} controls/mutants: PASS', flush=True)
