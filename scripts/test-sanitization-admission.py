...
def test() -> None:
    with tempfile.TemporaryDirectory(prefix="brynja-sanitization-admission-") as temporary:
        ...
        replace(release, 'cumulative_milestones = ["0.11.0", "0.11.1", "0.11.2", "0.12.0", "0.13.0", "0.13.1", "0.13.2", "0.13.3", "0.14.0", "0.15.0"]',
                'cumulative_milestones = ["0.11.1"]')
        require_rejection(root, "cumulative milestone")
        copy_fixture(root)

        release = root / "release-crates.toml"
        replace(release, 'milestone = "0.15.0"', 'milestone = "0.11.2"')
