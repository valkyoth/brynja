...
INDEX = 'security/tuplehash-execution-native.json'
...
def sources(root):
    ...
    for package in (*shared.PACKAGES, 'brynja-hash-sha3', 'brynja-hash-tuple'):
    ...
    names.update(name for name in tracked if name.startswith('assurance/tuplehash-execution/') and
                 'target' not in name.split('/') and name.endswith(('.rs', '.toml', '.lock')))
    names.update(name for name in tracked if name.startswith('scripts/tuplehash/') and name.endswith('.py'))
    names.update(name for name in tracked if name.startswith('crates/brynja-hash-tuple/tests/'))
...
def record_check(record, lane, commit, expected):
    ...
    for mode in ('portable', 'prefer-portable', 'static', 'prefer'):
        ...
        marker = f'TupleHash execution oracle: PASS; mode={mode}; cases=268; kernel={route}'
        require(marker in results['package'].splitlines(), 'TupleHash actual keyed public operation execution')
    if kernel == 'ArmKeccak':
        require('TupleHash execution oracle: PASS; mode=hosted; cases=268; kernel=ArmKeccak' in
                results['package'].splitlines(), 'TupleHash hosted execution')
...
def validate(root=ROOT):
    ...
    require(isinstance(capture, str) and re.fullmatch('[a-f0-9]{40}', capture), 'TupleHash native collection pending')
    ...
    for lane, row in index['lanes'].items():
        ...
        name = row['artifact']
        require(isinstance(name, str) and re.fullmatch('assurance/tuplehash-execution-native/[a-z0-9_-]+[.]json', name), 'Keccak artifact path')
    ...
    print('Hardened TupleHash native evidence: PASS; project-owned, non-FIPS')
