"""Exact reviewed native hardened MD5 captures, never timing/migration proof."""
import hashlib
import json
import re
import subprocess
...
INDEX='security/md5-hardened-native.json'
...
def record_check(record,lane,commit,sources):
    ...
    require(record['native']=='operator-self-attested' and record['profile']=='hardened-source-owned','classification')
    ...
    require(set(results)=={'portable','hosted','static','packaged','codegen'},'all result commands')
    require(all(isinstance(v,str) and len(v)<2_000_000 for v in results.values()),'result bounds')
    require('MD5_HARDENED: '+LANES[lane]+'; cases=2048; actual SIMD' in results['static'].splitlines(),'actual static SIMD')
    ...
    require(complete('static',5) and complete('portable',5),'complete execution tests')
    hosted='None' if 'x86_64' in lane else 'Some(Aarch64Neon)'
    require('MD5_HOSTED_HARDENED: '+hosted in results['hosted'].splitlines(),'hosted route')
    require(complete('hosted',1),'hosted tests')
    require('Packaged hardened MD5: 19 ownership/classification negatives; 4 compiled algorithm/health mutants rejected' in results['packaged'].splitlines(),'packaged mutations')
    require('Packaged hardened MD5 cleanup: 5 compiled whole-region/Drop mutants rejected' in results['packaged'].splitlines(),'compiled cleanup mutations')
    require('MD5 hardened MIR/LLVM/SIMD assembly: PASS; 1.98.1; '+target+'; panic=abort' in results['codegen'].splitlines(),'native compiler abort')
    require('MD5 hardened MIR/LLVM/SIMD assembly: PASS; 1.98.1; '+target+'; panic=unwind' in results['codegen'].splitlines(),'native compiler unwind')
...
def validate():
    ...
    for lane,row in index['lanes'].items():
        ...
        require(re.fullmatch(r'assurance/md5-hardened-observations/v0\.24\.44/[a-z0-9_-]+\.json',row['artifact']) is not None,'artifact path')
    ...
    print('MD5 hardened SIMD native qualification: reviewed correctness with residual limits')
