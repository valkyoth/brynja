...
def validate(root):
    ...
    words += ['fifty-eight', 'fifty-nine', 'sixty', 'sixty-one', 'sixty-two', 'sixty-three', 'sixty-four', 'sixty-five']
...
def regressions(root):
    ...
    cases += [
        (DOCS[0], 'Status: sixty-five exact', 'Status: nine exact'),
        (DOCS[0], 'Rust in only sixty-five exact', 'Rust in only nine exact'),
