...
spec = importlib.util.spec_from_file_location('hardened_oracle', Path(__file__).with_name('check-hardened-sha256-batch-oracle.py'))
driver = importlib.util.module_from_spec(spec)
spec.loader.exec_module(driver)


def result(mode, stdout='01;-;02;03;-;04;05;06\n', count=1, status=0, calls=None):
    if calls is None:
        calls = 0 if mode == 'portable' else 3
    marker = (f'HARDENED_SHA256_BATCH: batches={count}; vector_calls={calls}; '
...
def compiled(lane):
    ...
    messages = [bytes([slot]) * 64 for slot in range(8)]
    data = ';'.join('sha256:512:' + message.hex() for message in messages) + '\n'
    expected = [';'.join(hashlib.sha256(message).hexdigest() for message in messages)]
    with tempfile.TemporaryDirectory(prefix='brynja-hardened-sha256-mutants-') as temporary:
        root = Path(temporary)
        ...
        binary = root / 'target/release/brynja-hardened-sha256-batch-fixture'
        ...
        try:
            ...
            source.write_text(original)
    print('Hardened SHA-224/256 batch fixture: four compiled cleanup/output/route/counter mutants rejected; restored source passed')
...
def main():
    ...
    for mode in ('portable', 'prefer', 'required'):
        ...
        cases = [result(mode, stdout=''), result(mode, stdout='00;-;02;03;-;04;05;06\n'),
                 result(mode, stdout='01;02;-;03;-;04;05;06\n'), result(mode, count=0),
    ...
    print(f'Hardened SHA-224/256 batch oracle result checker: {rejected} regressions rejected')
    if args.lane:
        compiled(args.lane)
