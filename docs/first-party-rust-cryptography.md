...
Sixteen exact private SHA-2/Keccak/legacy-SHA-1/MD5 Rust function definitions use the C calling convention
...
`keccak_hardened_batch/{x86,arm}/secret.rs`, plus the legacy SHA-1 crate's
`cpu/{x86_sha1,aarch64_sha1}/secret.rs`, and the legacy MD5 crate's
`cpu/{x86_secret,arm_secret}/kernel.rs`). They contain first-party Rust inline assembly, not
...
even inside these sixteen files. This is not an exception allowing foreign crypto.
