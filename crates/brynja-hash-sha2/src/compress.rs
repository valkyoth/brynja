...
pub(crate) fn compress(state: &mut [u32; 8], block: &[u8; 64]) {
    ...
    for index in 16_usize..64 {
        let first = small_sigma1(schedule_word(&schedule, index.saturating_sub(2)));
        let second = schedule_word(&schedule, index.saturating_sub(7));
        let third = small_sigma0(schedule_word(&schedule, index.saturating_sub(15)));
        let fourth = schedule_word(&schedule, index.saturating_sub(16));
        let value = first
            .wrapping_add(second)
            .wrapping_add(third)
            .wrapping_add(fourth);
...
fn schedule_word(schedule: &[u32; 64], index: usize) -> u32 {
