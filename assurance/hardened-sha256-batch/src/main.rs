...
use brynja_crypto_cpu_std::sha256_hardened_batch::{Authority, Mode};
use brynja_hash_sha2::hardened_batch::{Control, PublicDeclassification, Report, Workspace};
...
fn main() -> Result<(), String> {
    ...
    loop {
        ...
        let mut secret = [[0xa5; 32]; 8];
        let mut public = [[0xa5; 32]; 8];
    ...
    eprintln!(
        "HARDENED_SHA256_BATCH: batches={count}; vector_calls={total_vector}; profiles=secret,declassified,public; cleanup=drop,declassify"
    );
    Ok(())
}
