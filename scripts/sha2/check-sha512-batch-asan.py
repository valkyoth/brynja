...
def main():
    ...
    result = acceptance.run(['cargo', '+nightly-2026-09-11', 'test', '--locked', '--offline',
        '-p', 'brynja-hash-sha2', '--features', 'batch512-execution', '--test', 'batch512',
        '--target', target, '--', '--nocapture'], env=env)
    if 'SHA512_BATCH_VECTOR: ' + kernel + '; calls=' not in result.stdout: raise ValueError('actual SIMD absent')
    print('Native SHA-512-family batch ASan and enforced LeakSanitizer: PASS; ' + kernel)
