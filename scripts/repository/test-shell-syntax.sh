...
scripts/repository/check_shell_syntax.sh "$posix_script" "$bash_script"

for portable_script in \
    scripts/cpu/check-cpu-admission-fixture.sh \
    scripts/sha2/check-sha256-cpu-codegen.sh \
    scripts/sha2/check-sha256-cpu-qemu.sh; do
...
if scripts/repository/check_shell_syntax.sh "$invalid_script" >/dev/null 2>&1; then
    echo "shell syntax validator accepted invalid Bash syntax" >&2
    exit 1
fi

if scripts/repository/check_shell_syntax.sh "$missing_shebang" >/dev/null 2>&1; then
