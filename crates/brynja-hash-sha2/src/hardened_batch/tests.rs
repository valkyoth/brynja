...
fn oracle(input: &Input<'_>) -> Result<[u8; 32], Error> {
    let mut result = [0; 32];
    match input.algorithm {
        Algorithm::Sha224 => result[..28].copy_from_slice(
            crate::Sha224::new()
                .finalize_bits(input.bits)
                .map_err(|_| Error::Invariant)?
                .as_bytes(),
        ),
        Algorithm::Sha256 => result.copy_from_slice(
            crate::Sha256::new()
                .finalize_bits(input.bits)
                .map_err(|_| Error::Invariant)?
                .as_bytes(),
...
#[test]
fn portable_sha224_sha256_known_answers_and_output_drop() -> Result<(), Error> {
    let inputs = [
        Some(Input::new(
            Algorithm::Sha224,
    ...
    assert_eq!(out.algorithm(0), Some(Algorithm::Sha224));
    assert_eq!(out.algorithm(1), Some(Algorithm::Sha256));
