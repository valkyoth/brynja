...
def validate(root):
    ...
    if features != {'default': [], 'keccak-batch': ['static-execution'], 'static-execution': [], 'runtime-execution': ['static-execution'],
...
def regressions(root):
    ...
    cases += [
        (DOCS[0], 'Status: thirty-six exact', 'Status: nine exact'),
        (DOCS[0], 'Rust in only thirty-six exact', 'Rust in only nine exact'),
