...
def main():
    ...
    for key in ('RUSTFLAGS','CARGO_ENCODED_RUSTFLAGS','RUSTDOCFLAGS','CARGO_BUILD_TARGET'):
        env.pop(key,None)
    env.update(RUSTFLAGS='-C target-feature='+features,BRYNJA_REQUIRE_MD5_EXECUTION='1')
    output=host.run(['cargo','+1.98.1','test','--locked','--release','-p','brynja-legacy-md5',
        '--features','execution','--lib','--test','execution','--','--nocapture'],env)
    backend='legacy-x86_64-avx2-md5' if args.architecture=='x86_64' else 'legacy-aarch64-neon-md5'
    if 'MD5_OPERATIONAL: '+backend+'; cases=2048; normal-build SIMD' not in output.splitlines():
        raise ValueError('native MD5 CI did not execute the required operational kernel')
    if '3 passed; 0 failed' not in output: raise ValueError('native MD5 CI incomplete suite')
    print(output)
    print('Ordinary MD5 native CI: PASS; correctness only, not deployment qualification')
