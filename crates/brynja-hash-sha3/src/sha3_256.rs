...
use crate::{Fips202BitString, Sha3_256Digest, Sha3_256Error, sponge::Sponge};
...
impl Sha3_256 {
    ...
    pub fn check_additional_bits(&self, additional: u128) -> Result<(), Sha3_256Error> {
        self.0
    ...
    ) -> Result<Sha3_256Digest, Sha3_256Error> {
        let bits = u128::try_from(input.bit_len()).map_err(|_| Sha3_256Error::MessageTooLong)?;
        self.0
            .check_additional_bits(bits)
            .map_err(|()| Sha3_256Error::MessageTooLong)?;
        let (complete, partial) = input.split();
        self.update(complete)?;
        Ok(Sha3_256Digest::from_bytes(self.0.finalize_bits(partial)))
    }
}
