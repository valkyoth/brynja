...
const _: unsafe extern "win64" fn(&mut [u8; 576], &[u64; 24]) = permute;
...
fn permutation_and_actual_return_registers() {
    ...
    for case in 0..1024 {
        ...
        if case < 2 {
            scratch[..200].fill(if case == 0 { 0 } else { 255 });
        }
        let expected = reference(&scratch[..200]);
        ...
        assert_eq!(&scratch[..200], &expected, "permutation case {case}");
        assert_eq!(&scratch[200..], &[0; 376], "scratch residue case {case}");
    }
    for offset in 1..32 {
        ...
        unsafe { permute(scratch, &ROUND_CONSTANTS) };
        assert_eq!(&scratch[..200], &expected);
        assert_eq!(&scratch[200..], &[0; 376]);
        assert!(
            backing[..offset]
                .iter()
...
unsafe fn observe(scratch: &mut [u8; 576], snapshot: &mut [u8; 320]) {
...
unsafe fn observe(scratch: &mut [u8; 576], snapshot: &mut [u8; 176]) {
...
fn guarded_bounds() -> std::io::Result<()> {
    ...
    for placement in 0..4 {
        ...
        let expected = reference(&scratch.read::<576>(s_end)[..200]);
        // SAFETY: ISA gate, disjoint exact regions with inaccessible neighbors.
        unsafe { permute(scratch.bytes(s_end), words) };
        assert_eq!(&scratch.read::<576>(s_end)[..200], &expected);
        assert_eq!(&scratch.read::<576>(s_end)[200..], &[0; 376]);
    }
    std::println!("KECCAK_BOUNDS: 4 guarded placements; readonly constants: PASS");
    Ok(())
}
