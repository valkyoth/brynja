extern crate std;
use super::{compress::ROUND_CONSTANTS, kernel::scalar};
...
fn differential_and_return_registers() {
    ...
    for case in 0..1024 {
        ...
        assert_eq!(state, &expected); // Also preserves the unused upper state.
    ...
    std::println!(
        "SHA256_SCALAR: 1024 independent pairs; 32 unaligned placements; return-register cleanup PASS"
    );
}
...
fn fixed_memory_bounds() -> std::io::Result<()> {
    ...
    for bits in 0..16 {
        ...
        block.readonly()?;
        constants.readonly()?;
        let table = constants.read::<256>(k);
        assert_eq!(table.as_ptr().align_offset(4), 0);
        // SAFETY: Disjoint fixed buffers at page edges; initialized aligned
        // u32 table and input are read-only, with inaccessible neighbor pages.
        unsafe {
            scalar(
                state.bytes::<64>(s),
                block.read::<128>(b),
                scratch.bytes::<640>(w),
                &*table.as_ptr().cast::<[u32; 64]>(),
            )
        };
        assert_eq!(state.read::<64>(s), &expected);
        assert!(scratch.read::<640>(w).iter().all(|x| *x == 0));
    }
    std::println!("SHA256_SCALAR_BOUNDS: 16 placements; readonly input/table PASS");
    Ok(())
}
