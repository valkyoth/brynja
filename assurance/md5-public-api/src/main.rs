...
use brynja_legacy_md5::{BitString, HardenedMd5, PublicDeclassification, md5_bits};
use std::{
    error::Error,
...
fn invalid() -> io::Error {
    io::Error::new(io::ErrorKind::InvalidInput, "invalid bounded MD5 request")
}

fn main() -> Result<(), Box<dyn Error>> {
    brynja_md5_public_api_fixture::acceptance()?;
    ...
    loop {
        ...
        let digest = md5_bits(bits)?;
        let mut hardened = [0; 16];
