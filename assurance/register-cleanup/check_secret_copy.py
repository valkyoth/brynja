#!/usr/bin/env python3
"""Secret copy development evidence; not a release-gate change."""
...
ROOT = HERE.parents[1]
SOURCE = ROOT / 'crates/brynja-core/src/secret_memory_transfer.rs'
FIXTURE = HERE / 'secret-copy'
...
def inspect(text, arm):
    ...
    found = [text[m.end():labels[i+1].start() if i+1 < len(labels) else len(text)]
             for i, m in enumerate(labels) if '10copy_bytes' in m[1]
             and ('secret_memory_transfer' in m[1] or 'secret_copy_cleanup' in m[1])]
    if len(found) != 1:
    ...
    for marker in ('BEGIN', 'ERASE', 'END'):
        if body.count('BRYNJA_COPY_' + marker) != 1:
    ...
    allowed = (r'(?:ldr|str|ldrb|strb|mov|cmp|b(?:\.lo|\.hs)|cbz|cbnz|add|sub)\b'
               if arm else r'(?:movq|movb|movzbl|xorl|cmpq|testq|jb|jae|je|jne|addq|subq|incq|decq)\b')
    ...
    expected = ([f'mov x{i}, xzr' for i in range(4, 7)] + ['cmp xzr, xzr'] if arm
...
def prepare(directory):
    ...
    lib.write_text(lib.read_text().replace('../../../../crates/brynja-core/src/secret_memory_transfer.rs', 'native.rs'))
...
def controls(original, arm):
    ...
    changes = ([
        ('"str x4, [{destination}, x5]"', '"nop"'),
        ('"strb w4, [{destination}, x5]"', '"nop"'),
        ('"str x4, [{destination}, x5]"', '"eor x4, x4, #1", "str x4, [{destination}, x5]"'),
        ('"strb w4, [{destination}, x5]"', '"eor w4, w4, #1", "strb w4, [{destination}, x5]"'),
        ('"mov x6, {length}"', '"mov x6, {length}", "mov x6, xzr"'),
        ('"cbnz x6, 4b"', '"nop"'),
    ] if arm else [
        ('"mov [{destination} + rcx], rax"', '"nop"'),
        ('"mov byte ptr [{destination} + rcx], al"', '"nop"'),
        ('"mov [{destination} + rcx], rax"', '"xor rax, 1", "mov [{destination} + rcx], rax"'),
        ('"mov byte ptr [{destination} + rcx], al"', '"xor eax, 1", "mov byte ptr [{destination} + rcx], al"'),
        ('"mov rdx, {length}"', '"mov rdx, {length}", "xor edx, edx"'),
        ('"jne 4b"', '"nop"'),
    ])
...
def main():
    with tempfile.TemporaryDirectory(prefix='brynja-secret-copy-') as temporary:
            ...
            for target in targets:
                ...
                for release in (False, True):
                    ...
                    for marker in ('BEGIN', 'ERASE', 'END'):
                        ...
                        mutant = text.replace(prefix+'BRYNJA_COPY_'+marker, op+'\n'+prefix+'BRYNJA_COPY_'+marker)
                        assert mutant != text
                        try:
                            inspect(mutant, arm)
                        except ValueError:
                    ...
                    print(f'Secret copy assembly: {compiler} {target} release={release}: PASS', flush=True)
                    ...
                    for label, changed, success in controls(original, arm):
                        ...
                        if success:
                            if result.returncode or 'SECRET_COPY: 8512 length/alignment cases;' not in log or 'SECRET_COPY_BOUNDS: four independent placements;' not in log:
                        ...
                        count += 1
                    print(f'Secret copy real crate + runtime: {compiler} {target} release={release}; {count} controls/mutants: PASS', flush=True)
