...
pub(super) fn inject(fault: Fault, point: Boundary, owner: &Authority, cancel: &Cancellation) {
    ...
    match fault {
        ...
        Fault::CancelDuringComparison if point == Boundary::Compare => {
            super::super::tests::configure(super::super::tests::Fault::CancelAt(1));
        }
        _ => (),
    }
}
pub(super) fn observe(
    ...
    output: &[u8],
    owner: &Authority,
    session: &brynja_mac_kmac::execution::KeccakSession<'_>,
) {
...
fn wrong_operation_missing_features_and_unsupported_models_reject() {
    ...
    for kernel in [Kernel::X86Keccak, Kernel::ArmKeccak] {
        if let Err(error) = require_target() {
            assert!(
                matches!(CompiledSession::new(Algorithm::Kmac256(256), kernel, limits()), Err(found) if found == error)
            );
        } else if let Err(error) = kernel.check_compiled_target() {
            assert!(
                matches!(CompiledSession::new(Algorithm::Kmac256(256), kernel, limits()), Err(Error::Backend(found)) if found == error)
            );
        }
    }
}
