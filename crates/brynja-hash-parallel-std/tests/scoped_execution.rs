...
use brynja_hash_parallel_std::{
    CancellationToken,
    execution::{Config, Error, Preference, Request, in_place::Executor},
};
...
fn required_native_routes_are_real_or_fail_without_output() -> Result<(), Error> {
    ...
    match executor.hash_public(&request, &mut output, &mut scratch, &token) {
        Ok(report) => {
            ...
            eprintln!("SCOPED_PARALLELHASH_NATIVE_THREADS: executed");
        }
        Err(
            Error::Unavailable
            ...
            eprintln!("SCOPED_PARALLELHASH_NATIVE_THREADS: unsupported; no native evidence");
        }
    ...
    for identity in [
        ...
        match executor.hash_public(&request, &mut output, &mut scratch, &token) {
            Ok(report) => {
                ...
                assert_eq!(output, expected);
                for (root, leaves) in [
                    (Preference::RequireStatic, Preference::Portable),
                    (Preference::Portable, Preference::RequireStatic),
                ] {
                    let mixed = Executor::new(Config {
                        workers: 2,
                        max_leaves: 16,
                        root,
                        leaves,
                    })?;
                    let (secret, report) = mixed.hash_secret(&request, &mut output, &token)?;
                    assert_eq!(secret.expose(), expected);
                    assert_eq!(report.root.is_some(), root == Preference::RequireStatic);
                    assert_eq!(
                        report.accelerated_leaves,
            ...
            Err(
                ...
                assert!(
                    std::env::var_os("BRYNJA_REQUIRE_SCOPED_PARALLEL").is_none(),
                    "required static scoped lane unavailable"
                );
                assert_eq!(output, [0xa5; 32]);
