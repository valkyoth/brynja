#!/usr/bin/env python3
"""Repository binding and deterministic ledger for v0.24.4 CPU evidence."""
...
EXPECTED_ADMISSIONS_SHA256 = "c66e36ed948078336fd55c95a784084cde37230164bffc4595802fa8315cddb0"
BACKEND_FIELDS = {
...
EXPECTED_REASONS = {
    ...
    "x86-avx2": "keccak-candidate-local-correctness-observed-admission-evidence-incomplete",
    ...
    "aarch64-sha3-keccak": "keccak-candidate-qemu-correctness-observed-native-evidence-incomplete",
...
def validate_admissions(policy: dict, admissions: dict, boundary: dict) -> None:
    ...
    if admissions["schema"] != {
        "version": 4,
        "milestone": "0.24.4",
        "status": "seven-candidates-zero-backends-admitted",
    }:
...
def validate_repository_binding() -> None:
    ...
    command = "run: python scripts/cpu/test-cpu-evidence.py"
    if workflow.count(command) != 1:
        fail("native host CI does not run CPU evidence fixtures")
    for relative in (
        "scripts/cpu/cpu_evidence_schema.py",
        "scripts/cpu/cpu_evidence_policy.py",
        "scripts/cpu/check-cpu-evidence.py",
        "scripts/cpu/validate-cpu-evidence.py",
        "scripts/cpu/test-cpu-evidence.py",
        "scripts/cpu/cpu_evidence_fixture_tests.py",
        "scripts/cpu/cpu_evidence_run.py",
        "scripts/cpu/validate-cpu-evidence-run.py",
        "scripts/cpu/test-cpu-evidence-runner.py",
        "scripts/cpu/manage-cpu-evidence.py",
        "scripts/sha2/capture-sha256-cpu-native.sh",
        "scripts/sha2/check-sha256-assembly-instruction.sh",
        "scripts/cpu/check-cpu-admission-fixture.sh",
    ):
        path = ROOT / relative
        if not path.is_file() or path.is_symlink():
...
def build_ledger(policy: dict, admissions: dict) -> dict:
    ...
    return {
        "schema": {"version": 4, "milestone": "0.24.4"},
        "claims": {
