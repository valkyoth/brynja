#!/usr/bin/env python3
"""Scalar SHA1 development evidence; not a release-gate change."""
...
ROOT = HERE.parents[1]
SOURCE = ROOT / 'crates/brynja-legacy-sha1/src/compress/native.rs'
FIXTURE = HERE / 'sha1-scalar'
...
def inspect(text, arm):
    ...
    allowed = (r'(?:ldr|ldur|str|mov|movk|cmp|b(?:\.hs|\.ne)?|and|bic|orr|eor|add|rev|ror)\b'
               if arm else r'(?:movl|xorl|cmpl|jae|jmp|jne|andl|notl|orl|leal|imull|addl|shrl|roll|incl|bswapl)\b')
    ...
    expected = ([f'mov x{i}, xzr' for i in range(4, 13)] + ['cmp xzr, xzr'] if arm
                else [f'xorl %{r}, %{r}' for r in (*REGISTERS, 'r14d')])
...
def prepare(directory):
    ...
    lib.write_text(lib.read_text().replace('../../../../crates/brynja-legacy-sha1/src/compress/native.rs', 'native.rs')
                   )
...
def controls(original, arm):
    registers = tuple(f'x{i}' for i in range(4, 13)) if arm else REGISTERS
    ...
    changes = ([
        ('"cmp x11, #320", "b.ne 4b"', '"cmp x11, #316", "b.ne 4b"'),
        ('"ror w9, w4, #27"', '"ror w9, w4, #26"'),
        ('"ldur w10, [x12, #-56]"', '"ldur w10, [x12, #-52]"'),
        ('"add w4, w4, w9"', '"mov w4, w9"'),
        ('"str wzr, [{schedule}, x11]"', '"nop"'),
        ('"cmp x11, #320", "b.ne 9b"', '"cmp x11, #316", "b.ne 9b"'),
    ] if arm else [
        ('"cmp r14d, 320", "jne 4b"', '"cmp r14d, 316", "jne 4b"'),
        ('"rol edx, 5"', '"rol edx, 6"'),
        ('"xor edx, [{schedule} + r14 - 56]"', '"xor edx, [{schedule} + r14 - 52]"'),
        ('"add r8d, edx"', '"mov r8d, edx"'),
        ('"mov dword ptr [{schedule} + r14], 0"', '"nop"'),
        ('"cmp r14d, 320", "jne 9b"', '"cmp r14d, 316", "jne 9b"'),
    ])
...
def main():
    with tempfile.TemporaryDirectory(prefix='brynja-sha1-scalar-') as temporary:
        ...
        for compiler in ('1.90.0', '1.98.1'):
            ...
            for target in targets:
                ...
                for release in (False, True):
                    ...
                    print(f'Scalar SHA1 assembly: {compiler} {target} release={release}: PASS', flush=True)
                    ...
                    run(['cargo', '+'+compiler, 'rustc', '--locked', '--offline', '-p', 'brynja-legacy-sha1',
                         '--no-default-features', '--target', target, *(['--release'] if release else []),
                         '--lib', '--', '--emit=asm'], crate_env)
                    actual = list(Path(crate_env['CARGO_TARGET_DIR']).glob(f'{target}/*/deps/brynja_legacy_sha1-*.s'))
                    assert len(actual) == 1
                    ...
                    for label, changed, success in controls(original, arm):
                        ...
                        if success:
                            if result.returncode or 'SHA1_SCALAR: 1024 independent pairs;' not in log or 'SHA1_SCALAR_BOUNDS: 8 placements;' not in log:
                        ...
                        count += 1
                    print(f'Scalar SHA1 real crate + runtime: {compiler} {target} release={release}; {count} controls/mutants: PASS', flush=True)
