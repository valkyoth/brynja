...
def build_register(policy: dict | None = None, ledger: dict | None = None) -> dict:
    ...
    if any(surface["disposition"] == "implemented" for surface in surfaces):
        lib.fail("v0.3.1 must not classify any surface as implemented")
    return {
        "policy_sha256": lib.policy_hash(policy),
        "schema": 1,
