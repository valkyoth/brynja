...
pub(super) fn pack_bytes(
    ...
    // SAFETY: Typed arrays have exactly four contiguous lanes. The private
    // caller validated width=2/4 and cleared inactive workspace capacity.
    unsafe {
        transpose::<8, true>(
...
pub(super) fn commit_bytes(s: &Workspace, states: &mut [[u8; 64]; 4], width: usize) {
...
pub(super) fn commit(s: &Workspace, states: &mut [[u64; 8]; 4], width: usize) {
...
/// # Safety
/// Width <= 4. Both nonoverlapping buffers are live for their fixed layout:
/// packed WORDS rows of 32 bytes; unpacked four lanes of WORDS * 8 bytes.
...
pub(super) unsafe extern "C" fn transpose<const WORDS: usize, const PACK: bool>(
    ...
    // counters stay below WORDS and width; the eight-byte access fits exactly.
    ...
    unsafe {
        core::arch::asm!(
            ...
            source_lane = const if PACK { WORDS * 8 } else { 8 },
            destination_word = const if PACK { 32 } else { 8 },
            destination_lane = const if PACK { 8 } else { WORDS * 8 },
            out("rax") _, out("rcx") _, out("rdx") _, out("r8") _, out("r9") _,
            options(nostack),
        );
    }
    #[cfg(target_arch = "aarch64")]
    // SAFETY: Same fixed transposition bounds. X4 alone holds secret bytes and
    ...
    unsafe {
        core::arch::asm!(
            ...
            source_lane_shift = const if PACK { if WORDS == 8 { 6 } else { 7 } } else { 3 },
            destination_word_shift = const if PACK { 5 } else { 3 },
            destination_lane_shift = const if PACK { 3 } else { if WORDS == 8 { 6 } else { 7 } },
