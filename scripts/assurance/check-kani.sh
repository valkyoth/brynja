...
test "$harnesses" = "13" || {
    echo "Kani policy: expected exactly thirteen admitted SHA-2/SHA-3 harnesses, found ${harnesses}" >&2
    exit 1
}
...
if [ "$mode" = "--policy-only" ]; then
    echo "Kani policy: thirteen portable SHA-2/SHA-3 bounds are inventoried; full proofs are local tag-gate evidence"
    exit 0
fi
...
echo "Kani proof: cargo-kani ${kani_version} with Rust ${kani_toolchain}; thirteen SHA-2/SHA-3 bounds passed"
