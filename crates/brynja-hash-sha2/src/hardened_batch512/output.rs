...
/// ```compile_fail
/// use brynja_hash_sha2::hardened_batch512::SecretBatchOutput;
...
/// ```
/// ```compile_fail
/// use brynja_hash_sha2::{Sha512Digest, hardened_batch512::SecretBatchOutput};
/// fn implicit_public(out: SecretBatchOutput<'_>) -> Sha512Digest { out.into() }
/// ```
///
/// ```compile_fail
/// use brynja_hash_sha2::hardened_batch512::SecretBatchOutput;
...
pub struct SecretBatchOutput<'a> {
    ...
    pub(super) identities: [[u8; 2]; CAPACITY],
...
impl<'a> SecretBatchOutput<'a> {
    ...
    pub fn algorithm(&self, index: usize) -> Option<Algorithm> {
        Algorithm::from_code(u16::from_le_bytes(*self.identities.get(index)?))
    }
...
impl Drop for SecretBatchOutput<'_> {
    fn drop(&mut self) {
        ...
        let _ = clear_owned_region(self.identities.as_flattened_mut());
    }
}
