...
def validate(root):
    ...
    words += ['fifty-eight', 'fifty-nine', 'sixty', 'sixty-one', 'sixty-two', 'sixty-three']
...
def regressions(root):
    ...
    cases += [
        (DOCS[0], 'Status: sixty-three exact', 'Status: nine exact'),
        (DOCS[0], 'Rust in only sixty-three exact', 'Rust in only nine exact'),
