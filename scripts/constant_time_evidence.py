...
def validate(root: Path) -> None:
    ...
    compiler_step = (
        'run: scripts/check-constant-time-codegen.sh "${{ matrix.rust }}" '
        "x86_64-unknown-linux-gnu"
    )
    target_step = (
        'run: scripts/check-constant-time-codegen.sh 1.97.1 "${{ matrix.target }}"'
    )
    ...
    command = (
        "scripts/check-constant-time-codegen.sh 1.97.1 "
        "x86_64-unknown-linux-gnu"
    )
