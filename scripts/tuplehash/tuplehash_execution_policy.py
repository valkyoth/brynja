...
TOKENS = {
    ...
    'core_state.rs': ('state: State<', 'metadata: Metadata', 'Operation::new(self)',
        'fn append_bits(&mut self, byte: &u8, valid: u8)',
        ...
        'self.metadata.wipe();', 'let mut prefix = SecretEncodedInteger::empty();', 'prefix.left(bits)?;',
        'let mut suffix = SecretEncodedInteger::empty();', 'suffix.right(bits)?;',
