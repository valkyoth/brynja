...
Eight exact private SHA-2/Keccak Rust function definitions use the C calling convention
...
`sha256_hardened_batch/arm/secret.rs`). They contain first-party Rust inline assembly, not
...
even inside these eight files. This is not an exception allowing foreign crypto.
