...
pub(crate) fn compress(state: &mut [u64; 8], block: &[u8; 128]) {
    ...
    for (word, bytes) in schedule.iter_mut().take(16).zip(block.chunks_exact(8)) {
        if let [a, b, c, d, e, f, g, h] = bytes {
            *word = u64::from_be_bytes([*a, *b, *c, *d, *e, *f, *g, *h]);
        }
    }
    for index in 16_usize..80 {
        let value = small_sigma1(schedule_word(&schedule, index.saturating_sub(2)))
            .wrapping_add(schedule_word(&schedule, index.saturating_sub(7)))
            .wrapping_add(small_sigma0(schedule_word(
                &schedule,
                index.saturating_sub(15),
            )))
            .wrapping_add(schedule_word(&schedule, index.saturating_sub(16)));
    ...
    for (constant, word) in ROUND_CONSTANTS.iter().zip(schedule.iter()) {
        let first = h
            ...
            .wrapping_add(*word);
        let second = big_sigma0(a).wrapping_add(majority(a, b, c));
        h = g;
        g = f;
        f = e;
...
fn schedule_word(schedule: &[u64; 80], index: usize) -> u64 {
...
pub(crate) const fn choose(x: u64, y: u64, z: u64) -> u64 {
    (x & y) ^ ((!x) & z)
}

pub(crate) const fn majority(x: u64, y: u64, z: u64) -> u64 {
    (x & y) ^ (x & z) ^ (y & z)
}

pub(crate) const fn big_sigma0(value: u64) -> u64 {
    value.rotate_right(28) ^ value.rotate_right(34) ^ value.rotate_right(39)
}

pub(crate) const fn big_sigma1(value: u64) -> u64 {
    value.rotate_right(14) ^ value.rotate_right(18) ^ value.rotate_right(41)
}

pub(crate) const fn small_sigma0(value: u64) -> u64 {
    value.rotate_right(1) ^ value.rotate_right(8) ^ (value >> 7)
}

pub(crate) const fn small_sigma1(value: u64) -> u64 {
