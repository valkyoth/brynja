...
import sha256_batch_acceptance as oracle
native = oracle.load('hardened_oracle_host', 'capture-sha256-batch-native.py')

FIXTURE = ROOT / 'assurance/hardened-sha256-batch'
MARKER = (r'HARDENED_SHA256_BATCH: batches=(\d+); vector_calls=(\d+); '
...
def malformed(binary, env):
    slot = 'sha256:8:ff'
    requests = ('x', ';'.join(['sha256:1:01'] * 8), ';'.join(['sha384:0:'] * 8),
                ';'.join(['sha256:8:zz'] * 8), ';'.join(['sha224:8:0'] * 8),
                ';'.join(['sha256:32769:00'] * 8), ';'.join(['sha256:8:éé'] * 8),
                ';'.join(['sha256:8:'] * 8), ';'.join([slot] * 9), 'a' * 66001)
    ...
    print('Hardened SHA-224/256 batch fixture: ten malformed requests rejected')
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='brynja-hardened-sha256-oracle-') as directory:
        ...
        binary = Path(directory) / 'release/brynja-hardened-sha256-batch-fixture'
        for mode in ('portable', 'prefer', 'required') if features else ('portable',):
            ...
            # for required SIMD. Remaining cases have eight >=64-byte messages.
            ...
            print(f'Hardened SHA-224/256 batch oracle: PASS; batches={len(answers)}; mode={mode}; vector_calls={calls}', flush=True)
