        ...
        if path.suffix == ".rs":
            ...
            if relative in LOCAL_C_ABI:
                ...
                if relative == Path('crates/brynja-core/src/secret_memory_difference.rs'):
                    signature = ('pub(crate) unsafe extern "C" fn accumulate_byte(\n'
                                 '    difference: *mut u8,\n    left: *const u8,\n    right: *const u8,\n) {')
                if relative == Path('crates/brynja-core/src/secret_memory_transfer.rs'):
                ...
                if relative == Path('crates/brynja-legacy-md5/src/compress/native.rs'):
                    signature = ('pub(super) unsafe extern "C" fn scalar(\n'
                                 '    state: &mut [u8; 16],\n    block: &[u8; 64],\n'
                                 '    constants: &[u32; 64],\n    shifts: &[u32; 16],\n) {')
                if relative == Path('crates/brynja-legacy-sha1/src/compress/native.rs'):
                    signature = ('pub(super) unsafe extern "C" fn scalar(\n'
                                 '    state: &mut [u8; 20],\n    block: &[u8; 64],\n'
                                 '    schedule: &mut [u8; 320],\n) {')
                if relative.parent.name in ('compress32', 'compress64'):
                    signature = ('pub(super) unsafe extern "C" fn scalar(\n'
                                 '    state: &mut [u8; 64],\n    block: &[u8; 128],\n'
                                 '    scratch: &mut [u8; 640],\n    constants: &[u32; 64],\n) {')
                    if relative.parent.name == 'compress64':
                        signature = signature.replace('[u32; 64]', '[u64; 80]')
                if relative.name == 'transfer.rs':
                    ...
                    if relative.parent.name == 'keccak_hardened_batch':
                        signature = signature.replace('const WORDS: usize, ', '').replace('    swap: u32,\n', '')
                    if relative == Path('crates/brynja-legacy-md5/src/cpu/transfer.rs'):
                        signature = signature.replace('width: usize', 'lane: usize').replace('    swap: u32,\n', '')
