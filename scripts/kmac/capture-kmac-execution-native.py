#!/usr/bin/env python3
"""Capture one native hardened KMAC lane after a clean exceptional retest."""
import argparse
import json
import platform
from pathlib import Path
import sys
import kmac_execution_native as evidence
...
def capture(lane, output):
    ...
    results = {'package': host.execute([sys.executable, 'scripts/kmac/check-kmac-execution.py', option], env)}
    ...
    with output.open('x', encoding='utf-8') as stream:
        ...
        stream.write('\n')
    print(f'Native hardened KMAC capture: PASS; {lane}; commit={commit}')
