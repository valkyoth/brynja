...
use brynja_hash_tuple::{
    Fips202BitString, Fips202Output, TupleHash128, TupleHash256, TupleHashXof128, TupleHashXof256,
};
...
fn evaluate(request: &str, line: usize, rendered: &mut String) -> Result<(), Box<dyn Error>> {
    ...
    scoped::check(algorithm, custom_input, &items, output_bits, &output, line)?;
    ...
    macro_rules! absorb {
        ($state:expr) => {{
            ...
            for (bytes, bits) in items {
                state
                    .push_item_bits(bit_string(bytes, *bits, line)?)
