...
def validate(root):
    ...
    words += ['seventy', 'seventy-one', 'seventy-two', 'seventy-three']
...
def regressions(root):
    ...
    cases += [
        (DOCS[0], 'Status: seventy-three exact', 'Status: nine exact'),
        (DOCS[0], 'Rust in only seventy-three exact', 'Rust in only nine exact'),
