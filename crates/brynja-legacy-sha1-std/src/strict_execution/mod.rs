//! Opt-in protected scalar legacy SHA-1; qualification and independent retest pending.
//!
//! SHA-1 is collision-broken. This compatibility-only memory profile does not
...
//!
//! ```no_run
//! use brynja_legacy_sha1_std::strict_execution::{Error, Limits, Session};
...
/// Preacquired resources for one fixed public legacy SHA-1 identity.
///
...
/// ```compile_fail
/// fn bound<T: Send>() {}
/// bound::<brynja_legacy_sha1_std::strict_execution::Session>();
/// ```
/// ```compile_fail
/// fn bound<T: Sync>() {}
/// bound::<brynja_legacy_sha1_std::strict_execution::Session>();
...
/// ```compile_fail
/// use brynja_legacy_sha1_std::strict_execution::{Session, Digest, Error};
/// fn escape(s: &mut Session) -> Result<Digest<'static>, Error> { s.hash(b"abc") }
/// ```
/// A live digest loan prevents concurrent reuse:
/// ```compile_fail
/// use brynja_legacy_sha1_std::strict_execution::{Session, Error};
...
impl Session {
    ...
    pub fn new(limits: Limits) -> Result<Self, Error> {
        ...
        let output = ProtectedBytes::new(20, limits.max_output_mapping_bytes)?;
        Ok(Self {
            stack,
            output,
            limits,
...
/// ```compile_fail
/// fn bound<T: Send>() {}
/// bound::<brynja_legacy_sha1_std::strict_execution::Digest<'static>>();
/// ```
/// ```compile_fail
/// fn bound<T: Sync>() {}
/// bound::<brynja_legacy_sha1_std::strict_execution::Digest<'static>>();
