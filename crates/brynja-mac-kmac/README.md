...
`hardened_in_place::Kmac128Workspace` and `Kmac256Workspace` retain the sponge
and verification scratch in caller-owned storage. Construct them without secrets,
then use `with`/`with_bits` to borrow before absorbing a key. The callback's
