...
def validate(root):
    ...
    words += ['eighty-one', 'eighty-two', 'eighty-three']
...
def regressions(root):
    ...
    cases += [
        (DOCS[0], 'Status: eighty-three exact', 'Status: nine exact'),
        (DOCS[0], 'Rust in only eighty-three exact', 'Rust in only nine exact'),
