#!/usr/bin/env python3
"""Capture current-source hardened MD5 operations, never historical admission."""
import argparse
...
def capture(args):
    ...
    env=dict(os.environ)
    for key in ('RUSTFLAGS','CARGO_ENCODED_RUSTFLAGS','RUSTDOCFLAGS','CARGO_BUILD_TARGET','BRYNJA_REQUIRE_HARDENED_MD5'):
        env.pop(key,None)
    env['RUSTUP_TOOLCHAIN']='1.98.1'
    test=['cargo','+1.98.1','test','--locked','--offline','--release']
    results={}
    results['portable']=host.run(test+['-p','brynja-legacy-md5','--features','hardened-execution','--test','hardened_execution'],env)
    results['hosted']=host.run(test+['-p','brynja-legacy-md5-std','--features','runtime-hardened-execution','--test','hardened_execution','--','--nocapture'],env)
    env['RUSTFLAGS']='-C target-feature='+features
    env['BRYNJA_REQUIRE_HARDENED_MD5']='1'
    results['static']=host.run(test+['-p','brynja-legacy-md5','--features','hardened-execution','--test','hardened_execution','--','--nocapture'],env)
    results['packaged']=host.run([sys.executable,'scripts/md5/check-md5-package.py','--hardened'],env)
    results['codegen']=host.run([sys.executable,'scripts/md5/check-md5-hardened-codegen.py','--native'],env)
    ...
    print(f'Native hardened MD5 capture: PASS; {args.lane}; commit={commit}')
