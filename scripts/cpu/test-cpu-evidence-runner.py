...
def main() -> int:
    capture_text = Path("scripts/sha2/capture-sha256-cpu-native.sh").read_text(
        encoding="utf-8"
    )
    ...
    instruction_check = Path("scripts/sha2/check-sha256-assembly-instruction.sh")
    with tempfile.TemporaryDirectory() as temporary:
    ...
    for path in (
        Path("scripts/cpu/manage-cpu-evidence.py"),
        Path("scripts/cpu/cpu_evidence_run.py"),
        Path("scripts/cpu/validate-cpu-evidence-run.py"),
        Path("scripts/cpu/test-cpu-evidence-runner.py"),
        instruction_check,
    ):
