#!/usr/bin/env python3
"""Native hardened AVX2 batch ASan/LSan, never a portable-only success."""
import os
import platform
import subprocess
...
def main():
    ...
    subprocess.run(['cargo','+nightly-2026-09-11','test','--locked','--offline',
        '-p','brynja-legacy-md5','--features','hardened-execution','--lib','--test','hardened_execution',
        '--target','x86_64-unknown-linux-gnu'],cwd=ROOT,env=env,check=True,timeout=300)
    print('Native hardened MD5 AVX2 ASan and LeakSanitizer: PASS')
