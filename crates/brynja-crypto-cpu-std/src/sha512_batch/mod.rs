//! Default-off hosted independent-message SHA-512-family batching.
//!
...
use brynja_crypto_cpu::sha512_batch::Authority as CpuAuthority;
pub use brynja_hash_sha2::batch512::{
...
pub enum Error {
    ...
    Execution(brynja_hash_sha2::batch512::Error),
}
    ...
    pub fn executor(&self, minimum_common_blocks: usize) -> Result<Executor<'_>, Error> {
        if minimum_common_blocks == 0 {
            return Err(Error::Execution(
                brynja_hash_sha2::batch512::Error::InvalidSelection,
            ));
        }
        match &self.owner {
...
fn backend(error: brynja_crypto_cpu::sha512_batch::Error) -> Error {
    Error::Execution(brynja_hash_sha2::batch512::Error::Backend(error))
}
