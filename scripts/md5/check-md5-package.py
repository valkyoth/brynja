...
def main():
    with tempfile.TemporaryDirectory(prefix='brynja-md5-package-') as temporary:
        ...
        manifest += '[dependencies]\nbrynja-legacy-md5 = { version="=0.1.0", default-features=false }\n[patch.crates-io]\n'
        for package, version in CLOSURE.items():
            manifest += f'{package} = {{ path="../unpacked/{package}-{version}" }}\n'
        (consumer / 'Cargo.toml').write_text(manifest)
        (consumer / 'src/lib.rs').write_bytes((ROOT / 'assurance/md5-public-api/src/lib.rs').read_bytes())
    ...
    print('MD5 packaged closure and external consumer: PASS; no upload')
