...
generated closure register records eight current and 75 planned secret owners
with fields, temporaries, lifecycle exits, mandatory core or admitted-adapter
cleanup, typed output handling, evidence, consumers, and residual risks.
Fifteen mutation classes and a standalone zero-dependency `no_std` fixture
reject missing coverage, forged hardened capabilities, ordinary-state
substitution, changed failure output, retained partial secret output, and
missed Drop or recoverable-unwind cleanup. This is design and assurance
