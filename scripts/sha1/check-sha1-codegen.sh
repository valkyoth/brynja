...
def scoped(data):
    prefix = 'crates/brynja-legacy-sha1/src/hardened_in_place.rs:'
    compact = lambda text: re.sub(r'\s+', '', re.sub(r'Storage(?:Live|Dead)\(_\d+\);', '', text))
    for owner in ('Sha1', 'Cleanup'):
        ...
        pattern = (r'(?P<p>_\d+)=(?:no_retag)?copy\(\(\*_1\)\.0:&mutowner::Sha1Owner\);'
                   r'_\d+=Sha1Owner::wipe\(move(?P=p)\)->\[return:' + successor + r',unwind(?:unreachable|continue)\];}')
        ...
        assert len(calls) == 1 and 'Sha1Owner' in calls[0] and 'wipe' in calls[0]
...
for extension, before, after in (
    ...
    ('mir', 'Sha1Owner::wipe(', 'Sha1Owner::omitted('),
    ('ll', '4wipe', '4skip'),
    ('s', '4wipe', '4skip'),
):
    ...
    try:
        ...
        continue
    raise AssertionError('scoped SHA-1 cleanup mutant survived')
print('Scoped SHA-1 guard/handle cleanup survives MIR/LLVM/assembly; four mutations rejected')
PY
