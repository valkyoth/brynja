...
INDEX = 'security/keccak-hardened-native.json'
...
def sources(root):
    ...
    for package in (*shared.PACKAGES, 'brynja-hash-sha3'):
...
def record_check(record, lane, commit, expected):
    ...
    if kernel == 'ArmKeccak':
        require('Hardened Keccak hosted Arm execution: PASS; kernel=ArmKeccak' in
                results['package'].splitlines(), 'Keccak hosted execution')
...
def validate(root=ROOT):
    ...
    require(isinstance(capture, str) and re.fullmatch('[a-f0-9]{40}', capture), 'Keccak native collection pending')
    ...
    for lane, row in index['lanes'].items():
        ...
        name = row['artifact']
        require(isinstance(name, str) and re.fullmatch('assurance/keccak-hardened-native/[a-z0-9_-]+[.]json', name), 'Keccak artifact path')
    ...
    print('Hardened Keccak native evidence: PASS; project-owned, non-FIPS')
