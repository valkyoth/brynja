...
The default-off `keccak-hardened-batch` feature similarly exports distinct
`keccak_hardened_batch::{Authority, Session, Workspace}` owners. AVX2 permutes
four independent states; little-endian AArch64 NEON permutes two. Use
`Session::permute` for word states or `permute_bytes` for canonical little-endian
