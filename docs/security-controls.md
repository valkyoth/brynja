...
Status: v0.3.2 normative-requirement foundation enforcement
...
| Normative requirements | Stable identifiers bind exact source, section, status, errata, strength, applicability, decision, mapping scope, owner, lifecycle, revision, residual risk, target, test, and evidence; immutable parent-history comparison, immutable released scope, governance-only global mapping, source/disposition/owner link consistency, deterministic bidirectional indexes, and 51 fixtures reject removals, stale revisions, illegal transitions, unrelated mappings, drift, weakened SHOULD decisions, missing anchors, repository-escaping symlink targets, and premature claims |
