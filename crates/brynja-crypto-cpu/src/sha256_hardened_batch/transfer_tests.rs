...
fn byte_and_word_transfers_preserve_all_lane_identities() -> Result<(), Error> {
    ...
    for _ in 0..64 {
        let mut words = [[0_u32; 8]; 8];
        let mut bytes = [[0; 32]; 8];
        let mut blocks = [[0; 64]; 8];
        for ((state, encoded), block) in words.iter_mut().zip(&mut bytes).zip(&mut blocks) {
            for (word, encoded) in state.iter_mut().zip(encoded.as_chunks_mut::<4>().0) {
                seed ^= seed << 13;
                seed ^= seed >> 7;
                seed ^= seed << 17;
                let [a, b, c, d, _, _, _, _] = seed.to_le_bytes();
            ...
            for byte_input in [false, true] {
                ...
                for (word, row) in s.schedule.iter().enumerate() {
                    for (lane, (actual, block)) in
                        row.as_chunks::<4>().0.iter().zip(&blocks).enumerate()
                    {
                        let actual = u32::from_ne_bytes(*actual);
                        let expected = if word < 16 && lane < width {
                            u32::from_be_bytes(
                                *block.as_chunks::<4>().0.get(word).ok_or(Error::Invariant)?,
                            )
                        } else {
                            0
                        };
                ...
                let mut result_words = [[0xa5a5_a5a5; 8]; 8];
                let mut result_bytes = [[0xa5; 32]; 8];
                    ...
                    .enumerate()
                {
                    for ((word, encoded), reversed) in result_words
                        .iter()
                        .zip(result_bytes.as_chunks::<4>().0)
                        .zip(state.iter().rev())
                    {
                        let expected = if lane < width { *reversed } else { 0xa5a5_a5a5 };
