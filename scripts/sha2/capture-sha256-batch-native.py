...
def host(lane):
    ...
    if lane.endswith('x86_64'):
        ...
        dedicated = all({'sha_ni', 'sse2'} <= row for row in flags)
        return vendor, '+avx,+avx2' + (',+sha,+sse2' if dedicated else '')
    ...
    if lane == 'apple-m2-aarch64':
        ...
        dedicated = acceptance.run(['sysctl', '-n', 'hw.optional.arm.FEAT_SHA256']).stdout.strip() == '1'
        return cpu, '+neon' + (',+sha2' if dedicated else '')
    ...
    dedicated = all('sha2' in row for row in flags)
    return 'operator-labelled AWS Arm; provider identity not authenticated', '+neon' + (',+sha2' if dedicated else '')
...
def capture(args):
    ...
    for key in ('RUSTFLAGS', 'CARGO_ENCODED_RUSTFLAGS', 'RUSTDOCFLAGS', 'CARGO_BUILD_TARGET', 'BRYNJA_REQUIRE_SHA256_BATCH'):
        env.pop(key, None)
    results = {}
    results['hosted'] = acceptance.run(['cargo', '+1.98.1', 'test', '--locked', '--offline', '--release',
        '-p', 'brynja-crypto-cpu-std', '--features', 'sha256-batch', '--lib', 'sha256_batch'], env=env).stdout
    results['portable'] = acceptance.run(['cargo', '+1.98.1', 'test', '--locked', '--offline', '--release',
        '-p', 'brynja-hash-sha2', '--features', 'batch-execution', '--test', 'batch'], env=env).stdout
    ...
    results['vector'] = acceptance.run(['cargo', '+1.98.1', 'test', '--locked', '--offline', '--release',
        '-p', 'brynja-hash-sha2', '--features', 'batch-execution', '--test', 'batch', '--', '--nocapture'], env=env).stdout
    kernel = 'Avx2' if args.lane.endswith('x86_64') else 'Neon'
    if 'SHA256_BATCH_VECTOR: ' + kernel + '; calls=' not in results['vector']: raise ValueError('actual vector execution absent')
    results['packaged_oracle'] = acceptance.run([sys.executable, 'scripts/sha2/check-sha256-batch.py', '--native', '--package'], env=env).stdout
    results['codegen'] = acceptance.run([sys.executable, 'scripts/sha2/check-sha256-batch-codegen.py'], env=env).stdout
    results['benchmark'] = acceptance.run(['cargo', '+1.98.1', 'run', '--locked', '--offline', '--release',
        '--manifest-path', 'assurance/sha256-batch/Cargo.toml', '--', 'required', '--benchmark'], env=env).stdout
    if results['benchmark'].count('SHA256_BATCH_BENCH:') != 8: raise ValueError('incomplete comparative benchmark')
    ...
    record = dict(schema=1, version='0.24.45', lane=args.lane, commit=commit, compiler=compiler,
        cpu=cpu, system=platform.system(), features=features, source_sha256=sources, results=results,
    ...
    print(f'Native ordinary SHA-224/256 batch capture: PASS; {args.lane}; commit={commit}')
