#!/usr/bin/env python3
"""SHA-224/256 batch correctness, package and opt-in native execution evidence."""
import argparse
import os
import re
...
def main():
    ...
    if args.native:
        ...
        marker = re.search(r'SHA256_BATCH_ACCEPTANCE: batches=128; vector_calls=([1-9][0-9]*)\b', result.stderr)
        if result.stdout.splitlines() != expected[256:] or not marker:
            raise ValueError('required SIMD oracle/execution failure')
        print('128 full-width required SIMD batches: PASS')
