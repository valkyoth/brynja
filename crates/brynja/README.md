...
Default hash APIs remain portable. Separate CPU packages expose default-off raw static and platform-limited [hosted execution authority](https://github.com/valkyoth/brynja/blob/main/docs/hosted-cpu-execution.md). Complete ordinary SHA-2 execution is available through opt-in leaf features; separate [hardened SHA-2 execution](https://github.com/valkyoth/brynja/blob/main/docs/sha2-hardened-execution.md) has passed its exceptional retest and matching native functional checks. The [acceleration plan](https://github.com/valkyoth/brynja/blob/main/docs/ACCELERATION_USABILITY_AUDIT.md) covers later families while retaining portable defaults and separate independent-review/FIPS claims. Default-off [ordinary SHA-3/SHAKE execution](https://github.com/valkyoth/brynja/blob/main/docs/sha3-ordinary-execution.md) awaits exceptional review and fresh high-level native observations.

Crates.io supplies **0.20.0**; the hash/MAC examples below require this repository's unpublished **0.24.35** APIs. For published foundation APIs: `cargo add brynja --no-default-features`. For unreleased APIs from a local checkout: `cargo add brynja --path /path/to/brynja/crates/brynja --no-default-features`.
...
SHA-2 covers SHA-224, SHA-256, SHA-384, SHA-512, SHA-512/224, and SHA-512/256; SHA-3/SHAKE covers SHA3-224, SHA3-256, SHA3-384, SHA3-512, SHAKE128, and SHAKE256. SP 800-185 portable acceptance passed at v0.24.16; final acceptance passed at v0.24.17. CPU candidates remain unadmitted.
...
| `brynja-hash-parallel` | Complete allocation-free ParallelHash128/256 and ParallelHashXOF128/256 APIs with hardened and caller-scheduled ownership |
