...
/// Caller-owned cumulative permutation budget and cancellation callback.
/// One scalar permutation costs one; one AVX2/NEON call costs four/two respectively.
...
impl<'a> Control<'a> {
    ...
    /// Cumulative permutation-equivalent work charged through this control.
    ...
    pub(super) fn charge(&mut self, permutations: u64) -> Result<(), Error> {
        self.poll()?;
        let remaining = self
            .remaining
            .checked_sub(permutations)
