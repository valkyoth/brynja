...
def validate(root):
    ...
    words += ['fifty-eight', 'fifty-nine', 'sixty', 'sixty-one']
...
def regressions(root):
    ...
    cases += [
        (DOCS[0], 'Status: sixty-one exact', 'Status: nine exact'),
        (DOCS[0], 'Rust in only sixty-one exact', 'Rust in only nine exact'),
