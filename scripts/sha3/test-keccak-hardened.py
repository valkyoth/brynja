...
def semantic():
    ...
    cases += [
        ...
        (policy.CPU + '/src/hardened_execution/keccak.rs',
         'dispatch(kernel, guard.scratch, state)?;',
         'guard.scratch.lanes.copy_from_slice(state); dispatch(kernel, guard.scratch, state)?;'),
        (policy.CPU + '/src/hardened_execution/keccak.rs',
         'dispatch(kernel, guard.scratch, state)?;',
         'dispatch(kernel, guard.scratch, state)?; state.copy_from_slice(&guard.scratch.lanes);'),
