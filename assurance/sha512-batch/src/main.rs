use brynja_hash_sha2::{BitString, Sha512Digest, batch512::*};
...
fn main() -> Result<(), String> {
    ...
    let authority =
        brynja_crypto_cpu_std::sha512_batch::Authority::new(mode).map_err(|e| format!("{e:?}"))?;
    ...
    loop {
        ...
        // Four slots, each at most 4096 bytes encoded as hex, plus metadata.
        (&mut reader)
            .take(66_001)
            .read_until(b'\n', &mut line)
        ...
        if line.len() > 66_000 || count >= 16384 {
        ...
        let mut storage: [Vec<u8>; 4] = core::array::from_fn(|_| Vec::new());
        ...
        for ((slot, bytes), meta) in slots.iter().zip(&mut storage).zip(&mut metadata) {
            ...
            let algorithm = match *algorithm {
                ...
                name if name.starts_with("t") => Algorithm::Sha512T(
                    brynja_hash_sha2::Sha512TBits::new(
                        name.get(1..)
                            .ok_or("t")?
                            .parse::<u16>()
                            .map_err(|e| e.to_string())?,
        ...
        let mut digests = [Some(Digest::Sha512(Sha512Digest::from_bytes([0xa5; 64]))); 4];
        ...
        for (index, digest) in digests.into_iter().enumerate() {
            ...
            match digest {
                Some(digest) => hex(&mut output, digest.as_bytes())?,
    ...
    eprintln!("SHA512_BATCH_ACCEPTANCE: batches={count}; vector_calls={total_vector}");
    Ok(())
}
