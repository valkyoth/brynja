//! Default-off clearing SHA-224/256 multibuffer compression owners.
//!
...
pub enum Kernel {
    /// Eight independent states, requiring AVX2 and OS-enabled YMM context.
    Avx2,
    /// Four independent states on little-endian AArch64 NEON.
    Neon,
}
impl Kernel {
...
///
/// ```compile_fail
/// use brynja_crypto_cpu::sha256_hardened_batch::Authority;
/// fn require<T: Send>() {}
/// require::<Authority>();
/// ```
/// ```compile_fail
...
impl Authority {
    ...
    fn create(kernel: Kernel, revalidate: fn(Kernel) -> bool) -> Result<Self, Error> {
        ...
        let blocks = [crate::sha256::abc_block(); 8];
...
///
/// ```compile_fail
/// use brynja_crypto_cpu::sha256_hardened_batch::Session;
/// fn require<T: Send>() {}
/// require::<Session<'_>>();
/// ```
/// ```compile_fail
